...
import com.bloomreach.xm.config.api.model.*;
...
public class Builder {
    ...
    public static AbstractComponent buildComponent(final HstComponentConfiguration componentConfig, final Session session) {
        if (componentConfig.getComponentType().equals(HstComponentConfiguration.Type.CONTAINER_COMPONENT)) {
            ...
            componentConfig.getChildren().values().forEach(childConfig -> {
     ...
     */
    public static Page buildPage(final HstComponentConfiguration componentConfig, final Session session) {
