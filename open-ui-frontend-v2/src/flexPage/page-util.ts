...
import {createUUID, getId, isNotEmptyOrNull} from "../common/common-utils";
 ...
 **/
export function componentToNode(component: AbstractComponent | StaticComponent | Page | ManagedComponent, regenerateId: boolean = false) {
    const node =
        ({
    ...
    if (regenerateId && component.type === 'managed') {
        node.component.id = createUUID();
    }

    isNotEmptyOrNull(component.components) &&
    component.components != null &&
    component.components.forEach(child => node.children.push((componentToNode(child, regenerateId)) as ComponentTreeItem));
    return node;
}
